-- =====================================================================
-- RESCATE VE — Fix: Bucle infinito entre triggers de sincronización
-- Aplica en: Supabase > SQL Editor > Run
--
-- Problema: sync_traslado_to_ticket() actualiza tickets,
--           lo que activa sync_ticket_changes_to_traslado(),
--           que actualiza traslados, volviendo al inicio → stack overflow.
--
-- Solución: pg_trigger_depth() > 1 detiene la cadena recursiva.
--           Si el trigger ya está siendo llamado desde otro trigger,
--           retorna inmediatamente sin ejecutar más UPDATEs.
-- =====================================================================


-- ─────────────────────────────────────────────────────────────────────
-- 1. Función: Tickets → Traslados  (con guard anti-recursión)
-- ─────────────────────────────────────────────────────────────────────
...
begin
  -- GUARD: Si ya estamos dentro de un trigger (llamado desde sync_traslado_to_ticket),
  ...
  -- CASO B: Ticket interno que califica como traslado logístico
  else
...
-- ─────────────────────────────────────────────────────────────────────
-- 2. Función: Traslados → Tickets  (con guard anti-recursión)
-- ─────────────────────────────────────────────────────────────────────
...
begin
  -- GUARD: Si ya estamos dentro de un trigger (llamado desde sync_ticket_changes_to_traslado),
  -- no hacer nada para evitar el bucle infinito.
  IF pg_trigger_depth() > 1 THEN
    RETURN NEW;
  END IF;
...
-- ─────────────────────────────────────────────────────────────────────
-- 3. Recrear ambos triggers (sin cambios, solo para garantizar que están activos)
-- ─────────────────────────────────────────────────────────────────────
