...
import {
  computeReportesOperaciones,
  type InventarioRow,
  type ReportesOperaciones,
  type TicketRow,
} from "@/lib/operaciones-reportes";
...
/** Obtiene y calcula el reporte operacional completo (service role). */
export async function fetchReportesOperacionesData(): Promise<ReportesOperaciones> {
  ...
  if (process.env.TUIA911_API_KEY) {
    try {
      ...
      console.warn("[Reportes API] Tuia911 inventario omitido:", tuiaErr);
    }
  }

  return computeReportesOperaciones({
